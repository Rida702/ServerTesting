...
class UploadImages(APIView):
    def post(self, request):
        try:
            ...
            print(5)
            # Save the images to the model
            my_model_instance = ImageModel(watch_image=watch_image_data,wrist_image=wrist_image_data,None)

            # Save the model instances
            my_model_instance.save()
            my_model_instance = ImageModel.objects.create(
                watch_image=watch_image_data,
                wrist_image=wrist_image_data,
                #result_image=result_image_io.getvalue()
            )
            print(6)
            context = {
