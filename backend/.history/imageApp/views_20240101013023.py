...
def upload_image(request):
    if request.method == 'POST':
        form = ImageUploadForm(request.POST, request.FILES)
        if form.is_valid():
            # Create a model instance without saving to the database yet
            uploaded_image = form.save(commit=False)

            # Convert the image to grayscale
            grayscale_path = grayscale_image(uploaded_image.image.path)

            # Save the original image to the model
            uploaded_image.image.save(os.path.basename(uploaded_image.image.path), File(open(uploaded_image.image.path, 'rb')))

            # Set the grayscale_image field directly with the grayscale image path
            uploaded_image.grayscale_image = f'media/grayscaled/grayscaled_{os.path.basename(uploaded_image.image.path)}'

            # Save the model to the database
            uploaded_image.save()

            # Introduce a sleep of 5 seconds
            time.sleep(5)
