...
def detect_wrist(image_path):
    ...
    wrist_boxes = [box for box, label, score in zip(boxes, labels, scores) if score > confidence_threshold and label == 1]
...
def overlay_watch(wrist_box, wrist_image_path, watch_image_path, output_path="output.jpg"):
    ...
    watch_image_1 = cv2.imread(watch_image_path)
    watch_image = remove(watch_image_1)
