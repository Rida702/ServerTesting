...
def overlay_watch(wrist_boxes, wrist_image_path, watch_image_path, output_path="output.jpg"):
    ...
    scale_factor = min(wrist_boxes[2] - wrist_boxes[0], wrist_boxes[3] - wrist_boxes[1]) / max(watch_h, watch_w)
    ...
    x_offset = int(wrist_boxes[0] + (wrist_boxes[2] - wrist_boxes[0] - watch_image.shape[1]) / 2)
    y_offset = int(wrist_boxes[1] + (wrist_boxes[3] - wrist_boxes[1] - watch_image.shape[0]) / 2)

    # Check if watch image has an alpha channel
    if watch_image.shape[2] == 3:  # No alpha channel
        wrist_image[y_offset:y_offset + watch_image.shape[0], x_offset:x_offset + watch_image.shape[1]] = watch_image
    else:  # Watch image has an alpha channel
        for c in range(0, 3):
            wrist_image[y_offset:y_offset + watch_image.shape[0], x_offset:x_offset + watch_image.shape[1], c] = (
                wrist_image[y_offset:y_offset + watch_image.shape[0], x_offset:x_offset + watch_image.shape[1], c] *
