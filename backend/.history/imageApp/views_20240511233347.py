    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if 'wristImage' in request.FILES and 'watchImage' in request.FILES:
                ...
                # Create an instance of UploadedImage model
                uploaded_image = ImageModel()
                print("----Saving images")
                # Save the uploaded images to temporary files
                with NamedTemporaryFile(suffix='.jpg', delete=True) as wrist_temp_file:
                    cv2.imwrite(wrist_temp_file.name, wrist_image)
                    uploaded_image.wrist_image.save('wrist_image.jpg', wrist_temp_file)

                with NamedTemporaryFile(suffix='.jpg', delete=True) as watch_temp_file:
                    cv2.imwrite(watch_temp_file.name, watch_image)
                    uploaded_image.watch_image.save('watch_image.jpg', watch_temp_file)

                with NamedTemporaryFile(suffix='.jpg', delete=True) as result_temp_file:
                    cv2.imwrite(result_temp_file.name, result_image)
                    uploaded_image.result_image.save('result_image.jpg', result_temp_file)
