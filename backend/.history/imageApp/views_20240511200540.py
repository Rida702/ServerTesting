...
def UploadImages(request):
    print("Inside upload images function")
    if request.method == 'POST':
        print("HERE 122")
        wrist_image = request.data.get('wristImage')
        watch_image = request.data.get('watchImage')

        # Handle saving the images to your desired location or performing any other operation
    ...
    return JsonResponse({'error': 'Only POST requests are allowed'}, status=403)
