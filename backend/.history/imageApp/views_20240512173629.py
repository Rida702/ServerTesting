...
from yourapp.models import ImageModel
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if 'wristImage' in request.FILES and 'watchImage' in request.FILES:
                ...
                watch_img = cv2.imread(watch_image_path)
                ...
                # Convert the OpenCV images to byte streams
                _, wrist_encoded = cv2.imencode('.jpg', wrist_img)
                wrist_content_file = ContentFile(wrist_encoded.tobytes())

                _, watch_encoded = cv2.imencode('.jpg', watch_img)
                watch_content_file = ContentFile(watch_encoded.tobytes())

                # Create an ImageModel instance
                image_model_instance = ImageModel()

                # Assign the converted images to the wrist_image and watch_image attributes
                image_model_instance.wrist_image.save('wrist_image.jpg', wrist_content_file, save=False)
                image_model_instance.watch_image.save('watch_image.jpg', watch_content_file, save=False)

                # Save the ImageModel instance to persist the changes
